...
## Plotting
ggplot(genre_counts,aes(x=age_certification , y=age_genre_count, fill = genres)) + geom_histogram(stat = "identity", position = "dodge") +
...
ggplot(genre_counts, aes(x = age_certification, y = genres, fill = age_genre_count)) +
...
top_10_movies <- titles %>% 
  ...
  kable(head(top_10_movies,10),
      align = "c",
      caption = "<b><center>HBO's top 10 movies from imdb_score",
      format = "html") %>% 
    kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
top_10_movies <- titles %>% 
  ...
  kable(head(top_10_movies,10),
      align = "c",
      caption = "<b><center>HBO's top 10 movies from tmdb_score" ,
      format = "html") %>% 
      kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
kable(head(top_10_movies,10) %>% 
      ...
      align = "c",
      caption = "<b><center>HBO's averaged top 10 movies from imdb_score and tmdb_score",
      format = "html") %>% 
      kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
ui = fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("range", "Select range of values: ", min= 0, max = 1000, value = c(0,500), step =50)
    ),
    mainPanel(
      plotOutput("map")
    )
