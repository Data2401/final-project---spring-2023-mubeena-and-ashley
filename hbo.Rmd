...
Today, we're going to take a deep dive into the world of HBO movies and TV shows.
...
Is there a correlations between age_restriction and genre?
...
Is there a correlation between Runtime of Movies and its Popularity?

Is there a correlation between Genre and Popularity?

Is there a correlation between the number of movies and shows produced and what country they come from?
...
titles %>% 
  count(type) %>%  
  kable(format = "markdown", col.names = c("Type", "Count")) %>% 
  kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
```{r Distribution of Genres table, echo=FALSE}
#Here's the table of number of genres in descending order
#kable(genre_counts, caption = "<b><center>Number of movies and TV shows by genre") %>% kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
## Plotting
ggplot(new_genre_counts,aes(x=age_certification , y=age_genre_count, fill = genres)) + geom_histogram(stat = "identity", position = "dodge") +
...
ggplot(new_genre_counts, aes(x = age_certification, y = genres, fill = age_genre_count)) +
  ...
  guides(fill = guide_colorbar(title = "Number of Titles", barwidth = 10, barheight = 0.5))
...
kable(unique_actors, caption = "Unique name for actors") %>% kable_styling(bootstrap_options = "bordered", full_width = FALSE)

kable(unique_directors, caption = "Unique name for directors") %>% kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
kable(head(actor_most_project, caption= "Actors with most projects"))%>% kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
kable(head(directors_most_project, caption= "Directors with most projects"))%>% kable_styling(bootstrap_options = "bordered", full_width = FALSE)
...
highiest_rated = both_data %>%  group_by(person_id,name, role,average_score,imdb_score,tmdb_score) %>%  
  summarize(mean_score = (average_score)) %>% ungroup() %>%
  arrange(desc(mean_score)) %>%  head()
...
## creating a new column in our titles.csv called decades: this will categorize what realease year as per their decades
...
```{r (shiny)genre_popularity,  echo=FALSE}
ui <- fluidPage(
...
```{r (unused) (shiny)countries, echo=FALSE}
## Defining UI
#  ui = fluidPage(
#    sidebarLayout(
#      sidebarPanel(
#        selectInput("type" , "Select type:" , c("Movies", "Tv Shows")),
#        sliderInput("range", "Select range of values: ", min= 0, max = 2000, value = c(0,500), #  step =50)
#      ),
#      mainPanel(
#        plotOutput("map")
#      )
...
# Remove rows that have country unknown
countries <- filter(countries, !is.na(production_countries) & production_countries != "") 

# Create a new column that represents the full country name of production countries
countries$full_country_name <- countrycode(sourcevar = countries$production_countries, origin = "iso2c", destination= "country.name")

# Rename "United States" to "USA" to match with data in world_map
countries <- countries %>% 
  mutate(full_country_name = if_else(full_country_name == "United States", "USA", full_country_name))
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("map", height = "500px", width = "800")
    )
  )
)
...
server <- function(input, output) {
  ...
  output$map <- renderPlot({
    ggplot(filtered_data(), aes(x = long, y = lat, group = group, fill = total)) +
      geom_polygon(aes(fill = total), color = "black",size = 0.01) +
      coord_equal() +
      scale_fill_gradient2(limits = c(0, 1824), low = "lightblue", mid = "blue", high = "darkblue", midpoint = 900) +
      labs(fill = "Count") +
      theme_light()
